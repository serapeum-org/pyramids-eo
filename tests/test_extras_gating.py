...
class TestHas:
    ...
    def test_returns_true_only_when_all_present(self):
        ...
        assert _has("os", "totally_missing_pkg_xyz") is False, "one missing fails the conjunction"

    def test_returns_false_for_missing_module(self):
        """An unimportable top-level name probes as unavailable."""
        assert _has("totally_missing_pkg_xyz") is False, "unknown module must be unavailable"
...
class TestExtraMarkers:
    ...
    def test_plot_marker_registered(self):
        ...
        assert isinstance(available, bool), f"availability must be bool, got {type(available)}"
...
class TestPytestCollectionModifyItems:
    ...
    def test_skips_marked_item_when_extra_unavailable(self, monkeypatch):
        ...
        monkeypatch.setattr("tests.conftest.EXTRA_MARKERS", {"plot": (False, "requires the [viz] extra")})
        item = _FakeItem({"plot"})
        pytest_collection_modifyitems(config=None, items=[item])
        assert len(item.added) == 1, f"exactly one skip marker expected, got {item.added}"
        assert item.added[0].name == "skip", f"marker must be skip, got {item.added[0].name}"
        assert item.added[0].kwargs["reason"] == "requires the [viz] extra", "skip reason must propagate"
    ...
    def test_does_not_skip_when_extra_available(self, monkeypatch):
        ...
        item = _FakeItem({"plot"})
        pytest_collection_modifyitems(config=None, items=[item])
        assert item.added == [], f"available extra must not add a skip, got {item.added}"

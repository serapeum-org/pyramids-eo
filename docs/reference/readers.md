...
per-granule calibration coefficients, and stitches the chunks by row position on
the geostationary grid (validated on real MTI1/Meteosat-12 granules — see issue
#40). `open_fci_l1c_chunk` is the lower-level radiance opener for use with the
generic `read_fci`.
